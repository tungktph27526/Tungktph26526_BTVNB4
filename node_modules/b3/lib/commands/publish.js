...
publisher.init = function (args, opts) {
  ...
  require("inquirer").prompt(prompts, function (answers) {
    _.assign(host, {
      cwd: cwd,
...
publisher.exec = function (args, opts) {
  ...
  var child = require("child_process").exec([].concat(conf.scripts.prebuild, "cd " + path.join(conf.cwd, conf.config), conf.scripts.build, "cd " + conf.cwd, conf.scripts.postbuild).join(" && "), {
  ...
  child.on("exit", function (code, signal) {
    var msg = "[INFO] b3 publish 以 code " + code + " 终止执行";
